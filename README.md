...
디자인 캡처를 Obsidian Markdown 볼트에 아카이빙하고, 작업창의 LLM이 분석과 wiki 지식층을 점진적으로 유지하며, 사람이 실행한 빌드로 생성된 JSON을 정적 웹사이트에서 검색·탐색·활용하는 사내 위키 프로젝트다.

현재 상태: **계획 문서 작성 단계**. 아직 애플리케이션 코드, 빌드 스크립트, 배포 설정은 만들지 않았다.

## 핵심 원칙

- Obsidian 볼트의 Markdown이 단일 진실 원천이다.
- 웹은 빌드된 JSON과 정적 자산만 읽는다.
- 이미지 분석, 인덱스 빌드 실행, 배포는 자동화하지 않고 사람이 작업창에서 요청하거나 실행한다.
